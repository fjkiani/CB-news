...
# AIHawk Job Application System

## System Architecture
```mermaid
graph TD
    subgraph Input Layer
        A[Excel Job List] --> B[Job Source Processor]
        L[LinkedIn API] --> B
        R[Resume YAML] --> C[Profile Manager]
    end

    subgraph Core Processing Layer
        B --> D[Job Analyzer]
        C --> D
        D --> E[Application Orchestrator]
    end

    subgraph AI Layer
        E --> F[LangChain Processor]
        F --> G1[Resume Customizer]
        F --> G2[Cover Letter Generator]
        F --> G3[Job Fit Analyzer]
    end

    subgraph Application Layer
        G1 --> H[Application Manager]
        G2 --> H
        G3 --> H
        H --> I1[LinkedIn Automator]
        H --> I2[Company Site Handler]
        H --> I3[Easy Apply Processor]
    end

    subgraph Storage Layer
        I1 --> J[Application Saver]
        I2 --> J
        I3 --> J
...
### 1. Input Layer
- **Job Source Processor**: Handles multiple job source inputs
  - Excel sheet processing
  - LinkedIn API integration
  - Indeed/other job board integrations
- **Profile Manager**: Manages user profiles and preferences
  - Resume parsing (YAML format)
  - Work preferences
  - Application history
...
## Technology Stack
- **Core**: Python 3.10+
- **AI/ML**: LangChain, LangGraph, Pydantic
- **Automation**: Playwright/Selenium
- **Storage**: Local filesystem, Optional DB
- **Processing**: RAG, Vector embeddings
- **API Integration**: LinkedIn API, Indeed API
